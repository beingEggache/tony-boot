...
(
    ...
    `use_scope`       smallint(1)  NOT NULL DEFAULT '1' COMMENT '使用范围: 1.全员, 2.指定人员（业务关联）, 3.均不可提交',
    `enabled`         tinyint(1)   NOT NULL DEFAULT '1' COMMENT '流程状态: 0.不可用, 1.可用',
    `model_content`   json         NOT NULL COMMENT '流程模型定义JSON内容',
    `sort`            smallint(1)  NOT NULL DEFAULT '0' COMMENT '排序',
...
(
    ...
    `variable`         json         NOT NULL COMMENT '变量json',
...
(
    ...
    `task_state`     smallint(1)  NOT NULL DEFAULT '1' COMMENT '任务状态: 1.活动, 2.完成, 3.拒绝, 4.超时, 5.终止',
    PRIMARY KEY (`task_id`) USING BTREE,
