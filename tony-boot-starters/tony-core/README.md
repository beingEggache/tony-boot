...
- **简单值请求结构（`MonoValue`）**
  用于只需上传单一参数（如手机号、验证码、ID 等）的接口，统一使用 `MonoValue` 包裹，避免重复定义 DTO。

- **列表请求结构（`RowsValue`）**
  用于批量上传列表数据（如批量 ID、批量对象等）的接口，统一使用 `RowsValue` 结构，提升批量操作接口的一致性。

- **分页请求结构**
  用于分页查询接口，统一包含分页参数、排序字段及查询条件。
    - 标准结构：使用 `PageQuery`，查询条件为嵌套对象。
    - 拉平结构：使用 `FlattenPageQuery`，将查询条件属性直接拉平到根节点，适用于需要扁平化参数的场景。

#### 统一响应结构

- **标准响应结构与拉平响应结构**
  所有接口返回统一的响应结构，包含状态码、消息、数据等字段。
    - 标准结构：使用 `ApiResult`，数据作为 data 字段返回。
    - 拉平结构：使用 `FlattenApiResult`，将数据属性直接拉平到响应根节点，适用于需要扁平化数据的场景。

- **简单值响应结构（`MonoResult`）**
  针对只返回单一数据的接口，提供简洁的响应结构，减少冗余字段。

- **分页响应结构（`PageResult`）**
  针对分页查询接口，统一返回分页数据、总数、当前页等信息，便于前端分页展示。

- **列表响应结构（`ListResult`）**
  针对仅返回列表的接口，统一使用列表结构，提升接口一致性。

- **树形结构响应（`TreeLike`, `ForestLike`）**
  用于需要返回树形或多层级结构数据的场景。
    - `TreeLike`：定义了树的递归结构。
    - `ForestLike`：扩展自 `TreeLike`，增加了节点编码、排序、根节点判断等能力，并提供了自动组装树结构的方法，适合组织、菜单、分类等多层级数据的统一返回。
