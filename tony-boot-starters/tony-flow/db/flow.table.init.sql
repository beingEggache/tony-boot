DROP TABLE `flow_task_cc` cascade;
DROP TABLE `flow_task_actor` cascade;
DROP TABLE `flow_task` cascade;
DROP TABLE `flow_instance` cascade;
DROP TABLE `flow_history_task_actor` cascade;
DROP TABLE `flow_history_task` cascade;
DROP TABLE `flow_history_instance` cascade;
DROP TABLE `flow_process` cascade;

CREATE TABLE `flow_task_cc`
(
    ...
    `task_state`     tinyint(1)   NOT NULL DEFAULT '1' COMMENT '任务状态 1，活动 2，结束',
...
(
    ...
    CONSTRAINT `fk_instance_process_id` FOREIGN KEY (`process_id`) REFERENCES `flow_process` (`process_id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_general_ci
...
(
    ...
    `task_name`      varchar(100) NOT NULL DEFAULT '' COMMENT '任务名称',
    `display_name`   varchar(200) NOT NULL DEFAULT '' COMMENT '任务显示名称',
    `task_type`      tinyint(1)   NOT NULL COMMENT '任务类型 1.主办 2.转办 3.委派 4.会签',
    `perform_type`   tinyint(1)   NOT NULL COMMENT '参与类型: 1-发起、其它, 2-按顺序依次审批, 3-会签, 4-或签, 5-票签, 10-抄送',
    ...
    CONSTRAINT `fk_task_instance_id` FOREIGN KEY (`instance_id`) REFERENCES `flow_instance` (`instance_id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_general_ci
...
(
    ...
    `updator_name`     varchar(50)  NOT NULL DEFAULT '' COMMENT '上次更新人',
    `update_time`      timestamp    NULL     DEFAULT NULL COMMENT '上次更新时间',
    `instance_state`   tinyint(1)   NOT NULL DEFAULT '1' COMMENT '流程实例状态: 1-活动, 2-完成, 3-超时, 4-终止',
    `end_time`         timestamp    NULL     DEFAULT NULL COMMENT '结束时间',
    PRIMARY KEY (`instance_id`) USING BTREE,
    KEY `idx_his_instance_process_id` (`process_id`) USING BTREE,
    CONSTRAINT `fk_his_instance_process_id` FOREIGN KEY (`process_id`) REFERENCES `flow_process` (`process_id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_general_ci
...
(
    ...
    `assignor`       varchar(100) NOT NULL DEFAULT '' COMMENT '委托人',
    `expire_time`    timestamp    NULL     DEFAULT NULL COMMENT '任务期望完成时间',
    `remind_time`    timestamp    NULL     DEFAULT NULL COMMENT '提醒时间',
    `remind_repeat`  tinyint(1)   NOT NULL DEFAULT '0' COMMENT '提醒次数',
    `viewed`         tinyint(1)   NOT NULL DEFAULT '0' COMMENT '已阅 0，否 1，是',
    `finish_time`    timestamp    NULL     DEFAULT NULL COMMENT '任务完成时间',
    `task_state`     tinyint(1)   NOT NULL DEFAULT '1' COMMENT '任务状态 1，活动 2，结束 3，超时 4，终止',
    PRIMARY KEY (`task_id`) USING BTREE,
    KEY `idx_his_task_instance_id` (`instance_id`) USING BTREE,
    KEY `idx_his_task_parent_task_id` (`parent_task_id`) USING BTREE,
    CONSTRAINT `fk_his_task_instance_id` FOREIGN KEY (`instance_id`) REFERENCES `flow_history_instance` (`instance_id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_general_ci
