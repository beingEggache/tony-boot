...
class UserService(
    ...
    fun login(req: UserLoginReq) =
        userDao.selectOne(QueryWrapper<User>()
    ...
    fun list(query: String?, page: Long = 1, size: Long = 10) =
        userDao.selectPage(Page(page, size), QueryWrapper<User>().like(!query.isNullOrBlank(),
            User.USER_NAME, query).or(!query.isNullOrBlank()) {
    ...
    fun add(req: UserCreateReq): Boolean {
        ...
        val exists = userDao.selectOne(QueryWrapper<User>()
            .eq(User.USER_NAME, req.userName)
        ...
        val userId = uuid()
        val insertUser = userDao.insert(User().apply {
            this.userId = userId
    ...
    fun update(req: UserUpdateReq): Boolean {
        ...
        return userDao.updateById(User().apply {
            this.userId = userId
            userName = req.userName
            realName = req.realName
            mobile = req.mobile
